...
/**
 * Minify stylesheets
 * Based on Reinhold Weber's method or Tubal Martin's class
 *
 * @package Packer
 * @author Vallo Reima
 * @copyright (C)2015
...
class PackCSS {
  ...
  private $mth = '';
  private static $fle = ['', true]; /* last filename, new file flag */
  public static $cnt = [0, 0];  /* calls counter: unique, total */
  ...
  private function process($css) {
    if ($this->mth == 'RW') { /* Reinhold Weber method */
...
class CSSmin {
  ...
  private $remove_last_semicolon; //VR
  ...
  public function __construct($raise_php_limits = TRUE, $remove_last_semicolon = TRUE) {
    ...
    $this->remove_last_semicolon = (bool) $remove_last_semicolon;
  }
  ...
  private function minify($css, $linebreak_pos) {
    ...
    if ($this->remove_last_semicolon) { //VR
